...
public class ToastUtil {
    ...
    public static void showToast(String text) {
        if (!TextUtils.isEmpty(text)) {
            if (mToast == null) {
                mToast = ToastCompat.makeText(BaseApplication.Companion.getContext(), text, Toast.LENGTH_SHORT);
            } else {
                mToast.cancel();
                mToast = ToastCompat.makeText(BaseApplication.Companion.getContext(), text, Toast.LENGTH_SHORT);
            }
    ...
    public static void showToastLong(String text) {
        if (!TextUtils.isEmpty(text)) {
            if (mToast == null) {
                mToast = ToastCompat.makeText(BaseApplication.Companion.getContext(), text, Toast.LENGTH_LONG);
            } else {
                mToast.cancel();
                mToast = ToastCompat.makeText(BaseApplication.Companion.getContext(), text, Toast.LENGTH_LONG);
            }
